...
static void UseCase_Handle(const UC_EVT_T *event_) {
  ...
  case UC_VIBRATION_DETECTED:
    printf("🔴 📳 [USECASE] Vibration Sensor (Alarm 2) ALARM -> sirens ON\n");
#if ENABLE_SIREN
    Siren_ControlAll(1);
#endif
    break;
  ...
  case UC_MOVEMENT_DETECTED:
    printf("🔴 🫨 [USECASE] Movement/Tilt Sensor (Alarm 1) ALARM -> Squawk\n");
#if ENABLE_SIREN
    Siren_ControlSquawk(0);
#endif
    break;
  case UC_MOVEMENT_CLEARED:
    printf("🟢 🧍 [USECASE] Movement/Tilt Sensor (Alarm 1) CLEARED\n");
    break;
  default:
    break;
  }
