...
static void UseCase_Handle(const UC_EVT_T *event_) {
    ...
    if (history != NULL) {
      ...
      if (g_sirenActive) {
        ...
        if (history->count == 3) {
          ...
          history->count = 0;
          if (diff <= 3.0) {
            printf("[USECASE] 3 presses in <= 3.0s -> turning sirens ON (Emergency Panic / FULL CAPACITY)\n");
#if ENABLE_SIREN
            Siren_SetMode(6);   // Mode 6 = Emergency Panic
  ...
  case UC_VIBRATION_DETECTED:
    printf("🔴 📳 [USECASE] Vibration Sensor (Alarm 2) ALARM -> sirens ON (Fire Panic)\n");
#if ENABLE_SIREN
    Siren_SetMode(5);
    Siren_ControlAll(1);
#endif
  ...
  case UC_VIBRATION_CLEARED:
    printf("🟢 📴 [USECASE] Vibration Sensor (Alarm 2) CLEARED\n");
    break;
  case UC_MOVEMENT_DETECTED:
    printf("🔴 🫨 [USECASE] Movement/Tilt Sensor (Alarm 1) ALARM -> sirens ON (Fire Panic)\n");
#if ENABLE_SIREN
    Siren_SetMode(5);
    Siren_ControlAll(1);
#endif
